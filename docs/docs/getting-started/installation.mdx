...
If you want to use an external postgresql instance, the environment variable set in `.env.prod` should look something like this:

`DATABASE_URL=postgres://db_user:db_pass@postgres_domain_or_ip:port/db_name`
